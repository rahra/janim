...
var rp_ =
{
   width: 1920, height: 1080,    //!< width and height
   ifps: 0,                      //!< input frame rate
   fps: 30,                      //!< output frame rate
   crf: 22,                      //!< compression rate
   outfile: "a.mkv",             //!< output filename
   vcodec: "",                   //!< video codec
   format: "",                   //!< container format
};
//! variable for spawned ffmpeg process
var ffmpeg;
...
{
   // parse CLI args
   var parser = new mod_getopt.BasicParser('c:(crf)f:(format)h(help)R:(ifps)r:(fps)v:(vcodec)W:(width)H:(height)', argv);
...
/*! This function initializes the fffmpeg process.
 */
function init_ffmpeg()
{
   ...
   ffmpeg = spawn("ffmpeg", ffopts_, {stdio: 'pipe'});
